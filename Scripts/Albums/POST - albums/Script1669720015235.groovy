...
GlobalVariable.id = '101'

response1 = WS.sendRequest(findTestObject('POST/POST - posts'))
...
response2 = WS.sendRequest(findTestObject('POST/POST - posts'))

WS.verifyResponseStatusCode(response2, 201)
WS.verifyElementPropertyValue(response2, 'title', 'Muzaki')
WS.verifyElementPropertyValue(response2, 'id', '101')